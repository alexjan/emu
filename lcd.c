...
extern bit bshow1, bshow2, bshow3, bshow4,blink;
...
void putBCDint(unsigned int VarBCDint) {
    ...
    if ((VarBCDint & 0xF000) && (bshow4  || blink)) {
        putch((unsigned char) ((VarBCDint >> 12) & 0x0F) + 0x30);
        Show = true;
    } else putch(' ');
    if(bshow3 || blink)putch((unsigned char) ((VarBCDint >> 8) & 0x0F) + 0x30);
    else putch(' ');
    putch(',');
    if(bshow2  || blink)putch((((unsigned char) VarBCDint >> 4) & 0x0F) + 0x30);
    else putch(' ');
    if(bshow1 || blink)putch(((unsigned char) VarBCDint & 0x0F) + 0x30);
    else putch(' ');
}
